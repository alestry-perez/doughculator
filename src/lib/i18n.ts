...
export const scheduleStrings = {
  en: {
    autolyse: 'Autolyse',
    autolyseNote: (flourG: number, waterG: number, restTime: string) => `Mix ${Math.round(flourG)}g flour and ${Math.round(waterG)}g water (hold back all salt and all starter). Cover and rest for ${restTime}.`,
    mix: 'Mix',
    mixNote: (autolyseOn: boolean, flourG: number, waterG: number, starterG: number, saltG: number, restTime: string) => autolyseOn
      ? `Add ${Math.round(starterG)}g starter and ${Math.round(saltG)}g salt to the autolysed dough. Mix until incorporated.`
      : `Combine ${Math.round(starterG)}g starter and ${Math.round(waterG)}g water, stir until dissolved. Add ${Math.round(flourG)}g flour and ${Math.round(saltG)}g salt to form a shaggy dough. Rest covered for ${restTime}.`,
    ...
    bulkNote: (minH: number, maxH: number) => `Ferment at room temp for ${minH}–${maxH}h until puffy.`,
    preShape: 'Preshape',
    preShapeNote: (restTime: string) => `Shape into a round ball. Rest covered for ${restTime}.`,
    roomProof: 'Final Shape & Room Proof',
    roomProofNote: (restTime: string) => `Shape and place into a floured proofing basket. Cover and rest for ${restTime}. Dough should puff and pass the poke test (slow spring-back).`,
    ...
    bakeCoveredNote: (restTime: string) => `Bake with lid on for ${restTime} — steam builds crust structure.`,
    handsOffRise: 'Hands-off Rise',
    handsOffRiseNote: (restTime: string) => `Leave dough undisturbed for ${restTime}. Watch for 50–75% volume increase.`,
    score: 'Score',
    scoreNote: (restTime: string) => `Use a lame or razor blade (${restTime}). Score just before loading into the oven.`,
    bakeUncovered: 'Uncovered (lid off)',
    bakeUncoveredNote: (restTime: string) => `Remove lid and bake for ${restTime} until deep golden brown.`,
    autolyseUserNote: (mins: number) => ` (your input: ${mins} min)`,
  },
  es: {
    autolyse: 'Autólisis',
    autolyseNote: (flourG: number, waterG: number, restTime: string) => `Mezcla ${Math.round(flourG)}g de harina y ${Math.round(waterG)}g de agua (reserva toda la sal e iniciador). Tapa y reposa por ${restTime}.`,
    mix: 'Mezclar',
    mixNote: (autolyseOn: boolean, flourG: number, waterG: number, starterG: number, saltG: number, restTime: string) => autolyseOn
      ? `Añade ${Math.round(starterG)}g de iniciador y ${Math.round(saltG)}g de sal a la masa autolizada. Mezcla hasta incorporar.`
      : `Combina ${Math.round(starterG)}g de iniciador y ${Math.round(waterG)}g de agua, mezcla hasta disolver. Agrega ${Math.round(flourG)}g de harina y ${Math.round(saltG)}g de sal para formar una masa irregular. Reposa tapado por ${restTime}.`,
    ...
    bulkNote: (minH: number, maxH: number) => `Fermenta a temperatura ambiente por ${minH}–${maxH}h hasta que esté esponjosa.`,
    preShape: 'Pre-formado',
    preShapeNote: (restTime: string) => `Forma una bola redonda. Reposa tapado por ${restTime}.`,
    roomProof: 'Formado Final & Fermentación Ambiente',
    roomProofNote: (restTime: string) => `Forma y coloca en el banneton enharinado. Tapa y reposa por ${restTime}. La masa debe hincharse y pasar la prueba del dedo (recuperación lenta).`,
    ...
    bakeCoveredNote: (restTime: string) => `Hornea con tapa por ${restTime} — el vapor forma la estructura de la corteza.`,
    handsOffRise: 'Reposo sin tocar',
    handsOffRiseNote: (restTime: string) => `Deja la masa sin tocar por ${restTime}. Observa un aumento de volumen del 50–75%.`,
    score: 'Greñar',
    scoreNote: (restTime: string) => `Usa una cuchilla o navaja (${restTime}). Greña justo antes de cargar en el horno.`,
    bakeUncovered: 'Destapado (sin tapa)',
    bakeUncoveredNote: (restTime: string) => `Retira la tapa y hornea por ${restTime} hasta que esté dorado oscuro.`,
    autolyseUserNote: (mins: number) => ` (tu elección: ${mins} min)`,
  },
  sv: {
    autolyse: 'Autolys',
    autolyseNote: (flourG: number, waterG: number, restTime: string) => `Blanda ${Math.round(flourG)}g mjöl och ${Math.round(waterG)}g vatten (håll tillbaka allt salt och all surdeg). Täck och vila i ${restTime}.`,
    mix: 'Blanda',
    mixNote: (autolyseOn: boolean, flourG: number, waterG: number, starterG: number, saltG: number, restTime: string) => autolyseOn
      ? `Tillsätt ${Math.round(starterG)}g surdeg och ${Math.round(saltG)}g salt till den autolyserade degen. Blanda tills inkorporerat.`
      : `Kombinera ${Math.round(starterG)}g surdeg och ${Math.round(waterG)}g vatten, rör tills upplöst. Tillsätt ${Math.round(flourG)}g mjöl och ${Math.round(saltG)}g salt och forma en grov deg. Vila täckt i ${restTime}.`,
    ...
    bulkNote: (minH: number, maxH: number) => `Jäs i rumstemperatur i ${minH}–${maxH}h tills degen är luftig.`,
    preShape: 'Förformning',
    preShapeNote: (restTime: string) => `Forma till en rund boll. Vila täckt i ${restTime}.`,
    roomProof: 'Slutformning & Rumstemperaturjäsning',
    roomProofNote: (restTime: string) => `Forma och lägg i mjölat jäskorg. Täck och vila i ${restTime}. Degen ska puffa upp och klara stickprovet (långsam återfjädring).`,
    ...
    bakeCoveredNote: (restTime: string) => `Baka med lock i ${restTime} — ångan bygger skorpans struktur.`,
    handsOffRise: 'Vila utan att röra',
    handsOffRiseNote: (restTime: string) => `Låt degen vila ostörd i ${restTime}. Titta efter 50–75% volymökning.`,
    score: 'Snitta',
    scoreNote: (restTime: string) => `Använd en lame eller rakblad (${restTime}). Snitta precis innan du laddar ugnen.`,
    bakeUncovered: 'Utan lock',
    bakeUncoveredNote: (restTime: string) => `Ta bort locket och baka i ${restTime} tills djupt gyllenbrun.`,
